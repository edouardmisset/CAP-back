...
const deleteAllDBData = async () => {
  ...
  try {
    ...
    for (const { tablename } of await db.$queryRaw(
      `SELECT table_name AS tablename FROM information_schema. tables WHERE table_type='BASE TABLE' AND table_schema='public' AND table_name<>'_prisma_migrations';`
    )) {
      await db.$queryRaw(`TRUNCATE TABLE "${tablename}" CASCADE;`)
    }
    for (const { relname } of await db.$queryRaw(
      ...
      await db.$queryRaw(`ALTER SEQUENCE "${relname}" RESTART WITH 1;`)
    }
  } catch (error) {
    console.log({ error })
  }
